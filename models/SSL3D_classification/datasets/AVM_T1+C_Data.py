...
class AVM_T1C_Data(Dataset):
    def __init__(self, root, split, fold=None, transform=None):
        super().__init__()
        self.split = split  # "train" | "val" | "test"
        self.img_dir = Path(root) / "nnsslPlans_onemmiso/Dataset003_AVM_T1+C/Dataset003_AVM_T1+C"
    ...
    def compute_sample_weights(self):  
        """  
        Compute weights based on class distribution or other sample characteristics.  
        """  
        # Example: Inverse class frequency weighting  
        labels = torch.tensor([self.train_dataset.labels[i] for i in range(len(self.train_dataset))])  
...
class AVM_T1C_DataModule(BaseDataModule):
